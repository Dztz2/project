...
while flag0==1 || flag1==1
    ...
    for i=1:n
        if ~isempty(new_node(i).Location)
            ...
            Y=new_node(new_node(i).Location(2)).Value;
            T=new_node(i).Lamda;
            [new_node(new_node(i).Location(1)).Lamda,new_node(new_node(i).Location(2)).Lamda]=downward(X,Y,T);
        end
    end
    %给leafnode排序
    Q=zeros(1,m);
    ...
    for i=1:m
        ...
        for j=1:k
            point(i*k-k+j)=tau(j);
            sigma(i*k-k+j)=P\f0+new_node(list(i)).Lamda(1)*P\pl+new_node(list(i)).Lamda(2)*P\pr;
        end
    end
    %积分近似
    ...
    jl(1)=0;jr(m)=0;
    for i=1:m-1
        jl(i+1)=jl(i)+new_node(Q(i)).Value(5)+new_node(Q(i)).Lamda(1)*new_node(Q(i)).Value(1)+new_node(Q(i)).Lamda(2)*new_node(Q(i)).Value(3);
        jr(m-i)=jr(m-i+1)+new_node(Q(m-i+1)).Value(6)+new_node(Q(m-i+1)).Lamda(1)*new_node(Q(m-i+1)).Value(2)+new_node(Q(m-i+1)).Lamda(2)*new_node(Q(m-i+1)).Value(4);
    end
    %解在节点的近似
    u=zeros(1,m*k);
    u0=zeros(1,m*k);
    ...
    for i=1:m*k
        L1=abs(f(point(i))-u(i));
        L2=abs(f(point(i))+u(i));
    end
    test=max(max(L1))/max(max(L2));
    if test>tol
        for i=1:m
            if new_node(list(i)).Monitor>=Sdiv
                [l,r]=new_node(list(i)).Name;
                ...
                new_node(n+1).Monitor=monitor(f,l,(l+r)/2,k);
                new_node(n+2).Monitor=monitor(f,(l+r)/2,r,k);
                n=n+2;
                flag0=1;
            end
        end
